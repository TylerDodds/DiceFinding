...
NUM_Y_ROTATION_CATEGORIES = 16
ROTATION_LOSS_SCALE = 1.0
...
def prediction_tensors_to_boxes(detection_scores, y_indices, x_indices,
                                ...
                                offset_predictions, y_rot_category_predictions):
  ...
  if y_rot_category_predictions is not None:
    y_rot_category_predictions_flat = _flatten_spatial_dimensions(y_rot_category_predictions)
    detected_y_rot_categories = tf.gather(y_rot_category_predictions_flat, peak_spatial_indices, batch_dims=1)
  else:
    detected_y_rot_categories = None

  return boxes, detection_classes, detection_scores, num_detections, detected_y_rot_categories
...
Y_ROTATION_CATEGORY = 'y_rotation_category'
...
class CenterNetMetaArch(model.DetectionModel):
  ...
  def _construct_prediction_heads(self, num_classes, num_feature_outputs,
    ...
    if self._od_params is not None:
      ...
      if NUM_Y_ROTATION_CATEGORIES is not None:
        prediction_heads[Y_ROTATION_CATEGORY] = [
          make_prediction_net(NUM_Y_ROTATION_CATEGORIES)
          for _ in range(num_feature_outputs)
        ]
  ...
  def _compute_object_detection_losses(self, input_height, input_width,
    ...
    """
    od_scale_loss, od_offset_loss, od_y_rot_loss = self._compute_box_scale_and_offset_loss_and_y_rotation_loss(
        scale_predictions=prediction_dict[BOX_SCALE],
        offset_predictions=prediction_dict[BOX_OFFSET],
        input_height=input_height,
        input_width=input_width,
        y_rotation_category_predictions = prediction_dict[Y_ROTATION_CATEGORY])
    loss_dict = {}
    loss_dict[BOX_SCALE] = (
        self._od_params.scale_loss_weight * od_scale_loss)
    loss_dict[BOX_OFFSET] = (
        self._od_params.offset_loss_weight * od_offset_loss)
    loss_dict['y_rotation'] = (od_y_rot_loss)
    return loss_dict

  def _compute_box_scale_and_offset_loss_and_y_rotation_loss(self, input_height, input_width,
                                         scale_predictions, offset_predictions, y_rotation_category_predictions):
    ...
    gt_y_rotation_angle_list = self.groundtruth_lists(additional_fields.InputDataFields.y_rotation_angle)
    num_boxes = _to_float32(get_num_instances_from_weights(gt_weights_list))
    ...
    batch_y_rot_angle =  tf.concat([tensor for tensor in gt_y_rotation_angle_list], axis = 0)
    batch_y_rot_angle_exists = tf.math.logical_not(tf.math.equal(batch_y_rot_angle, 0))
    batch_y_rot_angle_existant = batch_y_rot_angle[batch_y_rot_angle_exists]
    batch_y_rot_angle_existant = tf.expand_dims(batch_y_rot_angle_existant, -1)

    scale_loss = 0.0
    offset_loss = 0.0
    rotation_loss = 0.0
    localization_loss_fn = self._od_params.localization_loss
    zip_predictions = zip(scale_predictions, offset_predictions, y_rotation_category_predictions)
    for scale_pred, offset_pred, y_rot_cat_pred in zip_predictions:
      ...
      two_pi = 2.0 * np.pi
      y_rot_cat_pred_assigned = cn_assigner.get_batch_predictions_from_indices(y_rot_cat_pred, batch_indices)
      y_rot_cat_pred_assigned_existant = y_rot_cat_pred_assigned[batch_y_rot_angle_exists]
      #See the CenterNet paper's (https://arxiv.org/pdf/1904.07850.pdf) take on orientation loss, inspired by (arXiv:1811.10742) and (A. Mousavian, D. Anguelov, J. Flynn, and J. Kosecka. 3d bounding box estimation using deep learning and geometry. In CVPR, 2017)
      #See this other paper's (https://arxiv.org/pdf/2003.05597.pdf) example using many angle bins, with just categorization and and index function spreading across several bins
      y_rot_category_indices = tf.range(NUM_Y_ROTATION_CATEGORIES)
      y_rot_category_angles = tf.cast(y_rot_category_indices, tf.float32) * two_pi / NUM_Y_ROTATION_CATEGORIES
      batch_y_rot_angle_existant_difference_from_category = batch_y_rot_angle_existant - y_rot_category_angles
      batch_y_rot_angle_existant_distance_from_category = tf.math.abs(-np.pi + (batch_y_rot_angle_existant_difference_from_category + np.pi) % two_pi)
      HALF_WIDTH_DEG = 45
      HALF_WIDTH_RAD = HALF_WIDTH_DEG * np.pi / 180.0
      batch_y_rot_angle_existant_indicator_value_for_category = tf.maximum(1 - batch_y_rot_angle_existant_distance_from_category / HALF_WIDTH_RAD, 0)
      batch_y_rot_angle_existant_indicator_value_for_category_sum_per_batch = tf.expand_dims(tf.reduce_sum(batch_y_rot_angle_existant_indicator_value_for_category, axis = -1), -1)
      batch_y_rot_angle_existant_indicator_value_for_category /= batch_y_rot_angle_existant_indicator_value_for_category_sum_per_batch
      losses_from_y_cat_indicator = tf.keras.losses.categorical_crossentropy(batch_y_rot_angle_existant_indicator_value_for_category, y_rot_cat_pred_assigned_existant, from_logits = True)
      rotation_loss += losses_from_y_cat_indicator * ROTATION_LOSS_SCALE

    scale_loss = tf.reduce_sum(scale_loss) / (
        num_predictions * num_boxes)
    offset_loss = tf.reduce_sum(offset_loss) / (
        num_predictions * num_boxes)
    rotation_loss = tf.reduce_sum(rotation_loss) / (num_predictions * num_boxes)
    return scale_loss, offset_loss, rotation_loss
  ...
  def postprocess(self, prediction_dict, true_image_shapes, **params):
    ...
    boxes_strided, classes, scores, num_detections, y_rotation_categories = (
        prediction_tensors_to_boxes(
            detection_scores, y_indices, x_indices, channel_indices,
            prediction_dict[BOX_SCALE][-1], prediction_dict[BOX_OFFSET][-1], prediction_dict[Y_ROTATION_CATEGORY][-1]))

    boxes = convert_strided_predictions_to_normalized_boxes(
        boxes_strided, self._stride, true_image_shapes)

    y_rot_category_softmax = tf.nn.softmax(y_rotation_categories, axis = -1)
    y_categories_indices = tf.range(NUM_Y_ROTATION_CATEGORIES)
    y_categories_angles = tf.cast(y_categories_indices, tf.float32) * (2 * np.pi) / NUM_Y_ROTATION_CATEGORIES
    y_categories_sin = tf.math.sin(y_categories_angles)
    y_categories_cos = tf.math.cos(y_categories_angles)
    y_sin_weighted = y_rot_category_softmax * y_categories_sin
    y_cos_weighted = y_rot_category_softmax * y_categories_cos
    y_sin_sum = tf.reduce_sum(y_sin_weighted, axis = -1)
    y_cos_sum = tf.reduce_sum(y_cos_weighted, axis = -1)
    y_rotation_angles = tf.math.atan2(y_sin_sum, y_cos_sum)
    ...
    postprocess_dict = {
        ...
        additional_fields.DetectionResultFields.y_rotation_angles: y_rotation_angles,
    }
