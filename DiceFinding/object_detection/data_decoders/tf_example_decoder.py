...
class TfExampleDecoder(data_decoder.DataDecoder):
  ...
  def __init__(self,
    ...
    self.keys_to_features['image/object/rotation/y_angle'] = tf.VarLenFeature(tf.float32)
    self.items_to_handlers[additional_fields.InputDataFields.y_rotation_angle] = (slim_example_decoder.Tensor('image/object/rotation/y_angle'))

    self.keys_to_features['image/camera/intrinsic'] = tf.VarLenFeature(tf.float32)
    self.items_to_handlers[additional_fields.InputDataFields.camera_intrinsic] = (slim_example_decoder.Tensor('image/camera/intrinsic'))
    self.keys_to_features['image/camera/distortion'] = tf.VarLenFeature(tf.float32)
    self.items_to_handlers[additional_fields.InputDataFields.camera_distortion] = (slim_example_decoder.Tensor('image/camera/distortion'))
